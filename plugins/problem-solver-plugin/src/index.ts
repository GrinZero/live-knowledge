  ...
  configSchema = {
    ...
    properties: {
      systemPrompt: {
        ...
        description:
          "AI 解决问题时使用的核心指令模板。可以使用 {problem} 作为占位符。",
        format: "textarea",
      },
    },
  };
...
IMPORTANT: Use proper Markdown formatting. Use code blocks for code. Do NOT output raw HTML.`,
  ...
  private contextStore: Map<
    string,
    { problem: string; screenshotPath: string | null }
  > = new Map();

  private lastProblemContext: {
    problem: string;
    sessionId: string;
    timestamp: number;
  } | null = null;
  ...
  initialize(context: PluginContext) {
    ...
    context.http.router.post("/generate", async (req, res) => {
      ...
      console.log(
        `[Solver Plugin] Generating solution for: ${problem.substring(0, 50)}...`,
      );
      ...
      try {
        const promptTemplate =
          ...
          this.defaultConfig.systemPrompt;
...
4. Do NOT provide the full solution in the insight content immediately; offer to "Solve with AI".
    ...
    onAction: async (action: Action) => {
      if (action.type === "solve_problem") {
        ...
        const screenshotPath = action.payload.screenshotPath as
          | string
          | undefined;
        ...
        if (await this.isDuplicateTask(problem)) {
          console.log(
            "[ProblemSolverPlugin] Duplicate task detected, reusing existing session.",
          );
          if (this.solutionWindow && !this.solutionWindow.isDestroyed()) {
            this.solutionWindow.focus();
          } else {
            // If closed, reopen with same context?
            // Or create new session? Let's assume reopen logic isn't perfect yet and just open new
            // But ideally we should find the old session ID if possible
...
export default ProblemSolverPlugin;
