...
export default function SidebarPresentation(): React.JSX.Element {
  ...
  return (
    <div className="h-screen bg-transparent flex items-center justify-center p-2">
      <div className={`bg-gray-900 border-l-4 ${getPriorityColor(insight.priority)} rounded-r-lg shadow-2xl w-full max-w-sm transform transition-all duration-300 ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
        {/* Header */}
        ...
        <div className="p-3">
          ...
          {insight.metadata?.tags && (
            <div className="flex flex-wrap gap-1 mb-2">
              {insight.metadata.tags.map((tag: string, index: number) => (
                <span key={index} className="flex items-center space-x-1 px-2 py-1 bg-gray-800 text-xs rounded text-gray-300">
